...
class Handler(BaseHTTPRequestHandler):
    server_version = "OrbisCore/2.4"
    protocol_version = "HTTP/1.1"
    def log_message(self, fmt: str, *args: object) -> None:
        print(f"{self.client_address[0]} - {fmt % args}", flush=True)
    def send_bytes(self, body: bytes, content_type: str, status: int = 200, disposition: str = "") -> None:
        self.send_response(status); self.send_header("Content-Type", content_type); self.send_header("Content-Length", str(len(body))); self.send_header("Cache-Control", "no-store"); self.send_header("Connection", "close")
    ...
    def require_auth(self) -> bool:
        ...
        self.json_response({"error": "não autorizado"}, 401); return False
    ...
    def read_json(self, limit: int = 8192) -> dict[str, Any]:
        ...
        try:
            value = json.loads(self.rfile.read(length)); return value if isinstance(value, dict) else {}
        except json.JSONDecodeError: return {}
    def do_HEAD(self) -> None: self.do_GET()
    def do_GET(self) -> None:
        path = urllib.parse.urlsplit(self.path).path
        try:
            if path == "/health": self.send_bytes(b"ok\n", "text/plain; charset=utf-8")
            elif path in ("/", "/index.html"): self.send_bytes(dashboard_shell().encode(), "text/html; charset=utf-8")
            elif path == "/api/status": self.json_response(status_payload())
            elif path == "/api/log":
                if not self.require_auth(): return
                target = LOG_FILES.get(self.query().get("name", [""])[0])
                if not target: self.send_bytes(b"log invalido\n", "text/plain", 400); return
                lines = read_text(target, "sem log").splitlines()[-300:]; self.send_bytes(("\n".join(lines)+"\n").encode(), "text/plain; charset=utf-8")
            elif path == "/api/file-text":
                if not self.require_auth(): return
                name = self.safe_name(); target = SHARED_DIR / name
                if not name or not target.is_file() or target.suffix.lower() not in ALLOWED_TEXT_SUFFIXES or target.stat().st_size > MAX_TEXT_EDIT: self.json_response({"error": "arquivo não editável"}, 400); return
                ...
                if not command: self.json_response({"error": "comando não permitido"}, 400); return
                self.send_bytes((run(*command, timeout=12) + "\n").encode(), "text/plain; charset=utf-8")
            elif path.startswith("/files/"):
                name = Path(urllib.parse.unquote(path[len("/files/"):])).name; target = SHARED_DIR / name
            ...
            else: self.send_bytes(b"not found\n", "text/plain", 404)
        except Exception as exc: self.json_response({"error": type(exc).__name__, "message": str(exc)}, 500)
    def do_PUT(self) -> None:
        ...
        if path == "/api/files":
            ...
            if not name or length < 0 or length > MAX_UPLOAD: self.json_response({"error": "arquivo inválido ou maior que 25 MB"}, 400); return
            data = self.rfile.read(length); tmp = SHARED_DIR / ("." + hashlib.sha256(name.encode()).hexdigest()[:12] + ".tmp"); tmp.write_bytes(data); tmp.replace(SHARED_DIR / name); event(f"upload remoto: {name} ({length} bytes)"); self.json_response({"ok": True, "name": name})
        elif path == "/api/file-text":
            target = SHARED_DIR / name; length = int(self.headers.get("Content-Length", "0") or 0)
            if not name or target.suffix.lower() not in ALLOWED_TEXT_SUFFIXES or length > MAX_TEXT_EDIT: self.json_response({"error": "arquivo não editável"}, 400); return
            data = self.rfile.read(length); data.decode("utf-8"); tmp = SHARED_DIR / ("." + hashlib.sha256(name.encode()).hexdigest()[:12] + ".edit"); tmp.write_bytes(data); tmp.replace(target); event(f"arquivo editado remotamente: {name}"); self.json_response({"ok": True})
        else: self.json_response({"error": "rota inválida"}, 404)
    def do_DELETE(self) -> None:
        ...
        target.unlink(); event(f"arquivo excluído remotamente: {name}"); self.json_response({"ok": True})
    def do_POST(self) -> None:
        path = urllib.parse.urlsplit(self.path).path
        if not self.require_auth(): return
        data = self.read_json()
        if path == "/api/action":
            action = str(data.get("action", "")); commands = {"update": ["/usr/local/bin/orbis-update"], "restart-core": ["/usr/local/bin/orbis-web", "--restart"], "restart-remote": ["/usr/local/bin/orbis-remote", "--boot"], "sync-time": ["/usr/local/bin/orbis-time"], "reboot": ["reboot"], "poweroff": ["poweroff"]}; command = commands.get(action)
            if not command: self.json_response({"error": "ação não permitida"}, 400); return
            subprocess.Popen(command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True); event(f"ação remota iniciada: {action}"); self.json_response({"ok": True, "message": f"ação {action} iniciada"})
        elif path == "/api/job":
            name = Path(str(data.get("name", ""))).name; operation = str(data.get("operation", "")); job_file = JOBS_DIR / f"{name}.job"; enabled_file = JOBS_DIR / f"{name}.enabled"
            if not name or not job_file.is_file(): self.json_response({"error": "automação não encontrada"}, 404); return
            if operation == "toggle":
                if enabled_file.exists(): enabled_file.unlink()
                else: enabled_file.touch()
                event(f"automação alternada: {name}"); self.json_response({"ok": True, "message": "estado alterado"})
            elif operation == "run":
                subprocess.Popen(["/usr/local/bin/orbis-jobs", "--run", name], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True); event(f"automação executada remotamente: {name}"); self.json_response({"ok": True, "message": "execução iniciada"})
            else: self.json_response({"error": "operação inválida"}, 400)
        else: self.json_response({"error": "rota inválida"}, 404)

def main() -> None:
    server = OrbisHTTPServer((HOST, PORT), Handler)
    def shutdown(_signum: int, _frame: object) -> None: server.shutdown()
    signal.signal(signal.SIGTERM, shutdown); signal.signal(signal.SIGINT, shutdown); event(f"Orbis Core iniciado na porta {PORT}"); print(f"Orbis Core online em http://0.0.0.0:{PORT}", flush=True); server.serve_forever(poll_interval=0.5)
