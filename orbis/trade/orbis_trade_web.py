...
def db_summary() -> dict[str, Any]:
    if not DB.exists():
        return {"datasets": [], "runs": 0, "latest": None}
    ...
    try:
        ...
        if row:
            latest = dict(row)
            latest["params"] = json.loads(latest.pop("params_json"))
            latest["result"] = json.loads(latest.pop("result_json"))
        return {"datasets": datasets, "runs": runs, "latest": latest}
    finally:
        conn.close()
...
<section class='card'><div class='key'>Executar backtest SMA Cross</div><div class='form'><label>Ativo<input id='symbol' value='DEMO'></label><label>Timeframe<input id='timeframe' value='1h'></label><label>SMA rápida<input id='fast' type='number' value='9'></label><label>SMA lenta<input id='slow' type='number' value='21'></label><label>Capital (R$)<input id='capital' type='number' value='10000'></label><label>Taxa (bps)<input id='fee' type='number' value='10'></label></div><label>Token administrativo<input id='token' type='password' placeholder='Mesmo token do Orbis OS'></label><button onclick='runTest()'>Executar backtest</button><pre id='result'>Pronto.</pre></section>
...
const brl=new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'});token.value=localStorage.getItem('orbisToken')||'';token.onchange=()=>localStorage.setItem('orbisToken',token.value.trim());
function pct(v){return Number(v||0).toFixed(2)+'%'}
async function refresh(){const r=await fetch('/api/summary?x='+Date.now(),{cache:'no-store'});const d=await r.json();runs.textContent=d.runs;datasets.innerHTML=(d.datasets||[]).map(x=>`<p><b>${x.symbol}</b> · ${x.timeframe} · ${x.candles} candles</p>`).join('')||'<p>Nenhum dataset.</p>';const q=d.latest?.result;if(!q)return;initial.textContent=brl.format(q.initial_capital);final.textContent=brl.format(q.final_capital);ret.textContent=pct(q.return_pct);ret.className='value '+(q.return_pct>=0?'pos':'neg');dd.textContent=pct(q.max_drawdown_pct);win.textContent=pct(q.win_rate_pct);trades.innerHTML='<table><tr><th>Lado</th><th>Preço</th><th>Taxa</th><th>PnL</th></tr>'+(q.trades||[]).map(t=>`<tr><td>${t.side}</td><td>${Number(t.price).toFixed(4)}</td><td>${Number(t.fee||0).toFixed(2)}</td><td>${t.pnl==null?'—':brl.format(t.pnl)}</td></tr>`).join('')+'</table>'}
async function runTest(){result.textContent='Executando…';try{const body={symbol:symbol.value,timeframe:timeframe.value,fast:+fast.value,slow:+slow.value,capital:+capital.value,fee_bps:+fee.value};const r=await fetch('/api/backtest',{method:'POST',headers:{'Content-Type':'application/json','X-Orbis-Token':token.value.trim()},body:JSON.stringify(body)});const txt=await r.text();if(!r.ok)throw new Error(txt);result.textContent=JSON.stringify(JSON.parse(txt),null,2);await refresh()}catch(e){result.textContent='Falha: '+e.message}}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        if urllib.parse.urlsplit(self.path).path != "/api/backtest":
            self.send(b"not found\n", "text/plain; charset=utf-8", 404)
            return
        if not secrets.compare_digest(self.headers.get("X-Orbis-Token", ""), read_token()):
            self.send(json.dumps({"error": "não autorizado"}).encode(), "application/json", 401)
            return
        try:
