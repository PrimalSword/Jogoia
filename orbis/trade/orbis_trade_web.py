...
<style>
body{font-family:system-ui,sans-serif;background:#0b0f14;color:#eaf2ff;margin:0;padding:16px}main{max-width:1050px;margin:auto}.head{display:flex;justify-content:space-between;align-items:end;border-bottom:1px solid #526070;padding-bottom:14px}.online{font-size:28px;color:#8ef0b1}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px}.card{background:#141b24;border:1px solid #273444;border-radius:12px;padding:16px;margin-top:14px}.key{font-size:12px;color:#93a4b8;text-transform:uppercase}.value{font-size:22px;margin-top:6px;word-break:break-word}input,select,button{background:#0f151d;color:#fff;border:1px solid #40556d;border-radius:8px;padding:11px;box-sizing:border-box;width:100%}button{background:#1f4b37;width:auto;min-width:150px}button:disabled{opacity:.55}.form{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px}label{display:block;margin:8px 0}table{width:100%;border-collapse:collapse;font-size:14px}td,th{padding:8px;border-bottom:1px solid #273444;text-align:left}pre{white-space:pre-wrap;background:#080b0f;padding:12px;border-radius:8px;max-height:420px;overflow:auto}.pos{color:#8ef0b1}.neg{color:#ff9b9b}.muted{color:#93a4b8}a{color:#9dc8ff}.auth{display:flex;gap:8px;align-items:end;flex-wrap:wrap}.auth label{flex:1;min-width:230px}.chart{width:100%;height:230px;background:#0b1017;border:1px solid #273444;border-radius:10px}.note{font-size:13px;color:#b5c2d2;line-height:1.5}
</style></head><body><main>
<div class='head'><div><div class='key'>ORBIS FOREX LAB</div><div class='online'>BACKTEST / PAPER</div></div><a href='http://100.87.144.114:8080/'>Voltar ao Orbis OS</a></div>
<div class='grid'><section class='card'><div class='key'>Capital inicial</div><div class='value' id='initial'>—</div></section><section class='card'><div class='key'>Capital final</div><div class='value' id='final'>—</div></section><section class='card'><div class='key'>Retorno</div><div class='value' id='ret'>—</div></section><section class='card'><div class='key'>Drawdown</div><div class='value' id='dd'>—</div></section><section class='card'><div class='key'>Win rate</div><div class='value' id='win'>—</div></section><section class='card'><div class='key'>Pips líquidos</div><div class='value' id='pips'>—</div></section><section class='card'><div class='key'>Profit factor</div><div class='value' id='pf'>—</div></section><section class='card'><div class='key'>Execuções</div><div class='value' id='runs'>—</div></section></div>
...
<section class='card'><div class='key'>Backtest Forex — SMA + gestão de risco</div><div class='form'>
...
<label>SMA rápida<input id='fast' type='number' value='9' min='1'></label><label>SMA lenta<input id='slow' type='number' value='21' min='2'></label>
<label>Capital (R$)<input id='capital' type='number' value='10000' min='1' step='100'></label><label>Risco por operação (%)<input id='risk' type='number' value='1' min='.01' max='10' step='.1'></label>
<label>Stop (pips)<input id='stop' type='number' value='30' min='.1' step='.1'></label><label>Alvo (pips)<input id='take' type='number' value='60' min='.1' step='.1'></label>
<label>Spread (pips)<input id='spread' type='number' value='1' min='0' step='.1'></label><label>Valor do pip/lote (R$)<input id='pipValue' type='number' value='10' min='.01' step='.01'></label>
<label>Lote máximo<input id='maxLots' type='number' value='10' min='.01' step='.01'></label></div>
<p class='note'>Modelo inicial: conta tratada na moeda de cotação e valor do pip informado manualmente. Nenhuma ordem real é enviada.</p>
<button id='runBtn' onclick='runTest()'>Executar backtest Forex</button><pre id='result'>Pronto.</pre></section>
<section class='card'><div class='key'>Desempenho das últimas execuções</div><svg id='chart' class='chart' viewBox='0 0 900 230' preserveAspectRatio='none'></svg><div id='history'></div></section>
<section class='card'><div class='key'>Datasets disponíveis</div><div id='datasets'>carregando…</div></section>
...
function pct(v){return Number(v||0).toFixed(2)+'%'}function err(t){try{return JSON.parse(t).error||t}catch(e){return t}}
function drawChart(recent){const svg=chart;svg.innerHTML='';const items=[...(recent||[])].reverse();if(!items.length){svg.innerHTML="<text x='30' y='115' fill='#93a4b8'>Nenhuma execução registrada.</text>";return}const vals=items.map(x=>Number(x.result.final_capital||0));const min=Math.min(...vals),max=Math.max(...vals),span=Math.max(1,max-min);const pts=vals.map((v,i)=>[35+i*(830/Math.max(1,vals.length-1)),195-((v-min)/span)*155,v]);const line=document.createElementNS('http://www.w3.org/2000/svg','polyline');line.setAttribute('points',pts.map(p=>p[0]+','+p[1]).join(' '));line.setAttribute('fill','none');line.setAttribute('stroke','#8ef0b1');line.setAttribute('stroke-width','4');svg.appendChild(line);pts.forEach(p=>{const c=document.createElementNS('http://www.w3.org/2000/svg','circle');c.setAttribute('cx',p[0]);c.setAttribute('cy',p[1]);c.setAttribute('r','6');c.setAttribute('fill','#9dc8ff');svg.appendChild(c)})}
async function refresh(){try{const r=await fetch('/api/summary?x='+Date.now(),{cache:'no-store'}),d=await r.json();runs.textContent=d.runs;datasets.innerHTML=(d.datasets||[]).map(x=>`<p><b>${x.symbol}</b> · ${x.timeframe} · ${x.candles} candles</p>`).join('')||'<p>Nenhum dataset.</p>';drawChart(d.recent);history.innerHTML=(d.recent||[]).map(x=>`<p>#${x.id} · ${x.symbol}/${x.timeframe} · ${x.strategy} · ${pct(x.result.return_pct)} · ${brl.format(x.result.final_capital)}</p>`).join('')||'<p>Nenhuma execução.</p>';const q=d.latest?.result;if(!q)return;initial.textContent=brl.format(q.initial_capital);final.textContent=brl.format(q.final_capital);ret.textContent=pct(q.return_pct);ret.className='value '+(q.return_pct>=0?'pos':'neg');dd.textContent=pct(q.max_drawdown_pct);win.textContent=pct(q.win_rate_pct);pips.textContent=q.net_pips==null?'—':Number(q.net_pips).toFixed(2);pf.textContent=q.profit_factor==null?'—':Number(q.profit_factor).toFixed(2);trades.innerHTML='<table><tr><th>Lado</th><th>Entrada</th><th>Saída</th><th>Lotes</th><th>Pips</th><th>PnL</th><th>Motivo</th></tr>'+(q.trades||[]).map(t=>`<tr><td>${t.side}</td><td>${Number(t.entry??t.price).toFixed(5)}</td><td>${Number(t.price).toFixed(5)}</td><td>${t.lots??'—'}</td><td>${t.pips??'—'}</td><td>${t.pnl==null?'—':brl.format(t.pnl)}</td><td>${t.reason??'—'}</td></tr>`).join('')+'</table>'}catch(e){result.textContent='Falha ao atualizar painel: '+e.message}}
async function verifyToken(){authBtn.disabled=true;authState.textContent='Validando…';try{const r=await fetch('/api/auth',{headers:{'X-Orbis-Token':token.value.trim()},cache:'no-store'}),txt=await r.text();if(!r.ok)throw new Error(err(txt));localStorage.setItem('orbisToken',token.value.trim());authState.textContent='Token válido. Backtests liberados.';authState.className='pos'}catch(e){authState.textContent='Token inválido: '+e.message;authState.className='neg'}finally{authBtn.disabled=false}}
async function runTest(){runBtn.disabled=true;result.textContent='Executando backtest Forex…';try{const body={symbol:symbol.value,timeframe:timeframe.value,fast:+fast.value,slow:+slow.value,capital:+capital.value,risk_pct:+risk.value,stop_pips:+stop.value,take_pips:+take.value,spread_pips:+spread.value,pip_value:+pipValue.value,max_lots:+maxLots.value};const r=await fetch('/api/backtest',{method:'POST',headers:{'Content-Type':'application/json','X-Orbis-Token':token.value.trim()},body:JSON.stringify(body)}),txt=await r.text();if(!r.ok)throw new Error(err(txt));const d=JSON.parse(txt);result.textContent=`SUCESSO · execução #${d.run_id}\n${d.symbol} · ${d.closed_trades} operações\nCapital final: ${brl.format(d.final_capital)}\nRetorno: ${pct(d.return_pct)}\nPips líquidos: ${d.net_pips}\nDrawdown: ${pct(d.max_drawdown_pct)}\nWin rate: ${pct(d.win_rate_pct)}\nProfit factor: ${d.profit_factor}`;await refresh()}catch(e){result.textContent='Falha: '+e.message}finally{runBtn.disabled=false}}
